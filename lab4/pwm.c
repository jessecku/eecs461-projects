...
void setPWM(int submodule, int frequency, float dutyCycle)
{
  ...
  cmax = (vuint16_t)(10000000/frequency -1);
...
void initPWM(int submodule, int frequency, float dutyCycle)
{
  ...
  mcPWM_A.OUTEN.B.PWMA_EN = 0; // Disable PWMA 
  mcPWM_A.OUTEN.B.PWMB_EN = 0b0011; // Enable PWMB 
...
void outputTorque(float torque) {
  ...
  if(dutyCycle < DC_LOWER_LIMIT)
    ...
    dutyCycle = DC_UPPER_LIMIT;
		...
		//reset PWM duty cylce
	setPWM(MOTOR_SUBMODULE, MOTOR_FREQUENCY, dutyCycle);
}