#
# File    : rappid_564xl_cw_flash.tmf
#
# Abstract:
...
# All Rights Reserved .
#
# $Log: rappid_564xl_cw_flash.tmf,v $
...
MODEL           = Final_Project
MODULES         = Final_Project_data.c FlexPWM_564xL_library.c adc0_config.c adc_564xl_library.c adc_init.c adc_trigger_init.c eTimer_564xL_library.c flexcan_564xl_library.c flexpwm_init.c freemaster_MPC56xx.c freemaster_appcmd.c freemaster_protocol.c freemaster_rec.c freemaster_scope.c freemaster_serial.c freemaster_sfio.c freemaster_tsa.c gpio_564xl_library.c intc_init.c intc_sw_vectable.c linflex_init.c linflex_siu_init.c rappid_main.c rappid_utils.c romcopy.c rtGetInf.c rtGetNaN.c rt_nonfinite.c siu_init.c sys_init.c sysclk_init.c vmod.c vmod_wrapper.c 
...
MAKECMD             = "C:\Program Files (x86)\MATLAB\R2011b\bin\win32\gmake"
HOST                = PC
BUILD               = yes
...
ADD_INCLUDES = \
	-IH:\EECS_461\project\Final_Project_rappid_rtw \
	-IH:\EECS_461\project \
	-IH:\EECS_461\project\FINA~W#5 \
...
TOOL_OPTS= -proc Zen 
COMPILE_TOOL_OPTS= -vle -DVLE_IS_ON -gdwarf-2 -fp spfp_only -spe_vector -char unsigned $(TOOL_OPTS) -gccincludes -nostdinc -nosyspath -ansi off -opt all -bool on -Cpp_exceptions off
...
LCF_FILE = ../MPC564xLFlash.lcf
else
LCF_FILE = MPC564xLFlash.lcf
endif
LINK_TOOL_OPTS= -gdwarf-2 -fp spfp_only -char unsigned $(TOOL_OPTS) -m _start -nostdlib -srec -lcf $(LCF_FILE) -romaddr 0x00003000 -rambuffer 0x00003000 
# TBA: -romaddr 0x00003000 -rambuffer 0x00003000
PLIB_TEMP = precomp
LIB_TEMP  = libs
LIB_SUFFIX = MW

TOOL_CFiles=intc_init.c intc_sw_vecttable.c rappid_utils.c sys_init.c sysclk_init.c
# TBA: bam_init.c - initialize Bootup Reset Config Half Word(RCHW)
# TBA: swt_init.c - initialize Software Watchdog Timer (SWT)
# TBA: romcopy.c  - copies data from ROM to RAM
ifeq ($(MAKEFILEBUILDER_TGT),1)
...
$(LIB_TEMP)/%.o: C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\Conversions/%.c
	@echo ### "C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\Conversions\$*.c"
	@$(COMPILE_TOOL) $(COMPILE_TOOL_OPTS) $(CFLAGS) -o $@ -c $<
$(PLIB_TEMP)/%.o: C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\Conversions/%.c
	@echo ### "C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\Conversions\$*.c"
	@$(COMPILE_TOOL) $(COMPILE_TOOL_OPTS) $(CFLAGS) -o $@ -c $<


$(LIB_TEMP)/%.o: C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\VehicleSim/%.c
	@echo ### "C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\VehicleSim\$*.c"
	@$(COMPILE_TOOL) $(COMPILE_TOOL_OPTS) $(CFLAGS) -o $@ -c $<
$(PLIB_TEMP)/%.o: C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\VehicleSim/%.c
	@echo ### "C:\RAppIDToolbox\rappid\UofMBlocks\UofM_blocks\blocks\VehicleSim\$*.c"
	@$(COMPILE_TOOL) $(COMPILE_TOOL_OPTS) $(CFLAGS) -o $@ -c $<
...
.PHONY: PHONY_pathdata
PHONY_pathdata: $(foreach f,$(filter-out $(LIB_SKIP),$(MODULES_pathdata)),$(LIB_TEMP)/$f)
	@echo ### Creating $@
	@$(foreach f,$(filter-out $(LIB_SKIP),$(MODULES_pathdata)),$(shell echo $(LIB_TEMP)\$f >> pathdata.rsp))
	@$(ARCHIVER_TOOL) -o pathdata.a $(LIBRARY_OPTS) @pathdata.rsp

pathdata.a:
	@$(BLANK)
	@echo ### Creating $@
	@if not exist $(LIB_TEMP) mkdir $(LIB_TEMP)
	@$(MAKECMD) PHONY_pathdata -f Final_Project.mk



# EOF: rappid_564xl_cw_flash.tmf
