...
#include "eTimer_564xL_library.h"
   ...
   *   Function:  etimer_init_pcr_in_out_564xL
     ...
     @return none
   */
  void etimer_init_pcr_in_out_564xL(uint8_t subMod, uint8_t channel, uint8_t
    pcrNum)
  {
    if (subMod == 0) {
  ...
  void etimer_init_564xL(
    uint16_t subMod, uint16_t channel, uint16_t CNTMODE, uint16_t OUTMODE,
    uint16_t ONCE,
    ...
    uint16_t CLC2, uint16_t CMPMODE, uint16_t CPT1MODE, uint16_t CPT2MODE,
    uint16_t ONESHOT,
    uint16_t CFWM, uint16_t CNTRinit, uint16_t COMP1init, uint16_t COMP2init,
    uint16_t FILT,
    uint16_t INTDMA
    )
  {
